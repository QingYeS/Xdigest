...
6. **内容卡采用全量搬运**。内容卡展示 Serenity 单条推的完整英文原文 +
   完整中文直译,不做节选、概括或精简。账号核心价值是信息缺口的完整搬运,
   「干货全」优先于「图片轻量」。长推自动分页(render_tweet_cards),
   单推分页数由 MAX_CARDS_HARD 控制,默认上限 16 张。
   此为已确认的产品方向,后续文案打磨不得偏离。
   （版权边界问题单独留待后续评估,本条只锁定内容形态。）
